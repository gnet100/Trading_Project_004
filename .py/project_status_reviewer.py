...
class ProjectStatusReviewer:
    ...
    def parse_current_status(self, content):
        ...
        try:
            ...
            next_actions = next_actions_match.group(1).strip() if next_actions_match else "Not specified"
    ...
    def parse_tasks_status(self, content):
        ...
        try:
            ...
            last_completed = completed_matches[-1].strip() if completed_matches else "None found"
    ...
    def parse_session_archive(self, content):
        ...
        try:
            ...
            accomplishments_section = accomplishments_sections[-1] if accomplishments_sections else None
            
            if accomplishments_section:
                accomplishments = accomplishments_section.strip()
                # Take first 2 accomplishments
                accomplishment_lines = [line.strip() for line in accomplishments.split('\n') if line.strip()][:2]
    ...
    def analyze_project(self):
        ...
        current_status = self.parse_current_status(self.status_data.get("CURRENT_STATUS.md", ""))
        task_status = self.parse_tasks_status(self.status_data.get("TASKS.md", ""))
        session_info = self.parse_session_archive(self.status_data.get("SESSION_ARCHIVE.md", ""))
    ...
    def generate_summary(self, current_status, task_status, session_info):
        ...
        summary.append(f"   In Progress: {task_status.get('in_progress_count', 0)} tasks")
        summary.append("")
        
        # Last completed task
        summary.append("🏁 LAST COMPLETED:")
        last_completed = task_status.get('last_completed', 'None')[:80]  # Truncate if too long
...
def main():
    ...
    try:
        ...
        if not reviewer.md_path.exists():
            reviewer.safe_print(f"❌ Error: .md directory not found at {reviewer.md_path}")
            reviewer.safe_print("   Make sure you're running this script from the project root.")
            return
